...
packet formats in a common API.

Linux can support many devices with wildly different capabilities from
host NICs with only basic filtering functions up to full fledged silicon
...
The attributes are defined as follows,
    ...
    NET_FLOW_ACTIONS: Gives a list of supported actions and the arguments
                      the actions take.
...
used. Currently supported are 'exact', 'lpm' (least prefix match), and 'mask'.
